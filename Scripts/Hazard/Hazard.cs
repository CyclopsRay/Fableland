...
public abstract partial class Hazard : Area2D
{
    ...
    private IReadOnlyList<Shape2D> _runtimeShapes;
    private IReadOnlyList<Vector2> _runtimeShapePositions;
    private Rect2 _runtimeDebugBounds;

    /// <summary>
    /// Optional pre-Ready setup used by an orchestration-owned map runtime. It preserves the
    /// existing simple <see cref="BoxSize"/> contract for scene-authored hazards while allowing
    /// an authored map effect area (including a circle, polygon, or sub-cell mask) to remain
    /// exact at runtime. This API intentionally accepts only engine collision data, never map
    /// definitions, so Hazard stays in the GAMEPLAY layer.
    /// </summary>
    public void ConfigureRuntimeCollision(IReadOnlyList<Shape2D> shapes,
        IReadOnlyList<Vector2> shapePositions, Rect2 debugBounds)
    {
        if (shapes == null || shapePositions == null || shapes.Count == 0 || shapes.Count != shapePositions.Count)
        {
            GD.PushError("Hazard: runtime collision configuration is invalid.");
            return;
        }
        _runtimeShapes = shapes;
        _runtimeShapePositions = shapePositions;
        _runtimeDebugBounds = debugBounds;
        BoxSize = debugBounds.Size;
    }
    ...
    public override void _Ready()
    {
        ...
        if (_runtimeShapes == null)
        {
            AddChild(new CollisionShape2D { Shape = new RectangleShape2D { Size = BoxSize } });
        }
        else
        {
            for (int i = 0; i < _runtimeShapes.Count; i++)
                AddChild(new CollisionShape2D { Shape = _runtimeShapes[i], Position = _runtimeShapePositions[i] });
        }
    ...
    {
        var rect = _runtimeShapes == null ? new Rect2(-BoxSize / 2f, BoxSize) : _runtimeDebugBounds;
